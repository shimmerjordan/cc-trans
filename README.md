...
```bash
npm test        # 十四套件共 932 项,全部打本地 mock 上游
npm run test:ui # 另加:真浏览器里跑三个页面(170 项),没装 Chrome 会自动跳过
```
...
| `test/login.mjs` | 登录入口:节流器阶梯与双维度(IP / 账号)、根路径按 `Accept` 分流(浏览器给登录页、探针仍给 JSON)、统一入口签发的会话只在对应的门里有效、**走错门的指路**及其安全前提(密码不对时不泄露账号存在)、被禁用账号照实说、公告读写与鉴权、退出其它设备、管理台改密后别处失效而自己留着 |
| `test/chat2.mjs` | 聊天机制:**回合归服务端所有**(断线不丢回答、宽限期到点才取消、期内重连撤销计时器、`graceMs=0` 退回旧语义)、按序号续传与快照回退、显式停止 vs 断线可区分、同会话不并发(409 带可接回信息)、每人并发上限、分叉、改写截断、置顶与摘要、**改名/置顶不改排序时间**、AI 标题(生成 / 手改上锁)、上下文占用(实测 vs 估算 / `[1m]` 窗口)、**上游过载退避重试**(不重复开头、用尽后如实报错)、长对话才打缓存断点、删除正在生成的会话不会被落盘复活 |
| `test/usage.mjs` | 用量两层口径:有个人配额只回自己的额度(**响应里根本没有账户额度字段**)、没配额才回账户整体额度、金额跟着 `perms.cost` 走而**百分比与「额度用尽」告警不跟着走**、用户端传 `force=1` 也刷不动上游(只有管理台的刷新能穿透缓存)、两边共用同一份缓存、订阅接口挂掉时从限额头回落(**0~1 的比例换算成百分比** —— 混了会把 48% 画成 0%)、**网页聊天那条路也要采集限额头**(它不走 handleProxy,漏采的症状是只用网页聊天的人永远看不到额度) |
| `test/ui.mjs` | **真浏览器**(Chrome CDP,零依赖手写 WebSocket):统一入口点登录进管理台、创建用户时手输密码、「设置密码」对话框的三种校验、公告发布与用户端可见、聊天页真发一条消息并验 Markdown/代码块/思考折叠/操作条/上下文指示器/Artifacts 沙箱属性、搜索空态、改写与分叉、**用量条与用量弹窗**(条形宽度真的等于百分比、命中测试确认没被裁掉、抹掉金额后仍画得出百分比且不留 `null` 字样)、手机视口下侧栏能开能关且不横向溢出。**任何未捕获的页面异常都算失败** —— 这几个页面是零构建单文件 HTML,一个函数名写错的表现就是整页白屏,而接口测试一个都看不见 |
...
一个框两种账号:先按管理台凭证验,再按普通用户验,验完自动送进各自的门(管理员 → `/admin`,普通用户 → `/u/chat`)。发给同事时只需要给这一条链接,不必先解释"你是管理员就去 /admin、是普通用户就去 /u"。

直达 `/admin` 或 `/u` 照旧可用,而且**走错门会被明确指路**而不是收到一句"账号或密码错误":

| 情况 | 以前 | 现在 |
| --- | --- | --- |
| 普通用户在 `/admin` 登录 | `401 账号或密码错误`(以为自己记错密码) | `409` +「「zoe」是普通用户账号」+ 一个指向 `/u` 的链接 |
| 管理员在 `/u` 登录 | `401 账号或密码错误` | `409` + 指回 `/admin` 的链接 |
| 账号被禁用 | 与密码错误混在一起 | `403 账号已被禁用`(照实说) |
| 密码不对 | `401` | `401`,**通用错误**,不透露账号是否存在 |

最后一行是这套提示的安全前提:**只有在密码已经验证通过时**才说"这是普通用户账号"。密码不对就一律回落到通用错误,所以它不会变成账号存在性探测器。

根路径对**非浏览器**客户端(探活脚本、`docker healthcheck`、curl)仍然返回健康检查 JSON —— 只在 `Accept` 里明确要 HTML 时才给登录页,已有的监控一个都不用改。

**登录失败节流**:三个登录接口(`/api/login`、`/admin/api/login`、`/u/api/login`)共用一个节流器,按 **IP 和账号两个维度**各记一份。同一维度连续失败 5 次锁 30 秒,8 次锁 5 分钟,12 次锁 30 分钟;成功登录一次即清账。scrypt 让单次爆破变慢(~60ms),但并发 50 条连接照样是每秒近千次尝试 —— 慢不等于挡住了。两个维度都要:只按 IP 挡不住分布式撞库,只按账号挡不住"一个 IP 横扫所有账号"。
...
**账号密码登录**:用户名取 `adminUser`(默认 `admin`)。`adminPassword` 留空时,**首次启动会自动生成一个随机密码,打印到控制台并写回 config.json**——从 `journalctl -u cc-trans` 里能看到那段醒目的初始密码框。登录后可在**「设置 → 管理台账号」**或**「用户」页管理员那行的「编辑」**里改**登录名、备注和密码**(改登录名或密码要验当前密码,只想改一样就把另一样留空;改备注不用验——它不是凭证。改完写回 config.json)。会话 12 小时。

**改完密码,别处那些会话立刻失效**,只有发起改密的这一条被续上新凭证版本 —— 少了后半句,管理员改完自己的密码会当场被弹回登录页,那不是安全是 bug。密码下限与普通用户统一为 **8 位**(此前管理台只要 6 位,而它恰恰是权限最大的那个账号),并且不允许新旧密码相同。
...
- **用户**:创建普通用户账号并把**现有客户端令牌**分配给他(见下方「用户端」)。可**设置密码**、改绑设备、**配置权限**、**强制下线**、禁用/删除。用户密码用 **scrypt 哈希**存储,管理员也看不到明文。

  **「设置密码」**:输入框里**可以直接手输**,旁边的「随机生成」只是其中一个按钮 —— 此前这里只有一个「重置密码」,点下去直接生成一串随机字符,管理员无法指定密码;而当面告诉对方、沿用团队约定的初始口令、或者对方就是要一个自己记得住的,都是常事。对话框带二次确认、实时长度提示,以及一个默认**不勾选**的「保留该用户已登录的会话」:改别人的密码通常正是因为怀疑这个号被别人拿到了,旧会话留着等于只做了一半。保存后密码在可复制的对话框里显示一次(服务端只存哈希)。

  **「强制下线」**:不动密码,只把该用户所有登录会话作废,他重新登录即可继续用。

  这两件事都靠 `passVersion`(借鉴 sub2api 的 `TokenVersion`)实现:会话记住签发时的凭证版本,任何一次改密/作废都把版本 +1,旧会话下一个请求就对不上而失效。比"挨个去翻 session Map 删"可靠 —— 后者漏一处就是漏一处。「客户端」页也能反向看到每个令牌**归属哪个用户**。表格第一行是**管理员账号**。它是合成行(凭证在 `adminUser`/`adminPassword`,不在 `users` 数组里),但**渲染走和普通用户完全相同的一套单元格**——同样有备注、设备标签、创建时间、最近登录,点「编辑」弹出的对话框位置与字段也和「编辑用户」对齐。差别只有两处:
...
- **设置**:**公告**(写一句话,显示在用户端与聊天页顶部;自建服务里"今晚 23:00 重启"这类事此前只能靠口头通知,而这两个页面恰恰是所有人都会打开的地方。分普通/提醒/重要三档语气,每个用户可以关掉当前这条,内容改过之后重新出现,清空即撤下)+ **本地 AI 订阅配置**(订阅 OAuth ↔ 静态密钥切换、凭证路径检测、上游地址、代理,保存即热应用)+ 修改管理台账号。
...
**怎么开通一个用户**:管理台 →「用户」→ 创建用户(密码可手输,也可点「随机生成」)→ 勾选要分配的设备 → 创建。账号和初始密码会在对话框里显示一次(服务端只存哈希),把 `http://<本机IP>:8787/` 这条链接和账号密码一起发给对方即可 —— 他从统一入口登录就会直接落在聊天页。
...
- **用量与额度**:「我的设备」页顶部是用量合计;下面是额度 —— 管理员给你设了配额就显示自己的进度条,没设就显示**账户整体额度**(订阅的 5 小时 / 7 天窗口),见下文「用量:普通用户也看得到」。
- **我的请求**:只含自己设备的请求日志,分页 + 关键字/仅异常过滤。
- **账号**:自助改密码(要验旧密码,带显示/隐藏与长度提示)。改完**其它设备上的登录立即失效**,当前这台不受影响 —— 如果你是因为怀疑密码泄露才来改的,这正是你要的结果。同一页还有**登录设备**列表(IP、浏览器、登录时间,标出哪一条是当前设备)和一个**「退出其它设备」**按钮。
- **聊天**(`/u/chat`):见下节。
...
左栏会话列表、中间对话流、右栏 Artifacts 面板。**外观与交互都取自 [cc-haha](https://github.com/NanmiCoder/cc-haha)**(见文末「借鉴来源」)。

#### 外观:「纸 · 墨 · 印」

聊天页用的是 cc-haha 那套设计体系,不是 `/admin` 与 `/u` 共享的靛蓝那套:

- **赤陶印** `#96442B` 作主色,暖白纸底,**衬线标题**(标题是「墨」的那一半);
- **六套色板**:纯白 / 纸墨 / 暖旧 / 青瓷 / 暗色 / 靛墨,外加跟随系统。侧栏左上角切换;
- 侧栏是**浅色**、与正文同一族纸底,只靠一条发丝线分开(不是深色导航条);
- 用户消息是**右对齐、随内容收缩的浅底卡**(上限 82% / 78% / 640px),不是小气泡;
- 输入框是**带圆形赤陶发送键的大圆角卡**,模型 / 思考 / 深度 / 上下文占用都收在它内部的控件行里 —— 顶栏只留会话标题和一行元信息;
- 行内码是暖底小卡片,表格带圆角描边,引用块左侧一道赤陶边。

色板与语义层整块搬自 `cc-haha/desktop/src/theme/globals.css`,落在 `src/haha-tokens.css`:`--cc-*`(每套主题各一份原始值)→ `--color-*`(语义层只定义一次)。加一套主题 = 加一个 `--cc-*` 块,永远不动语义层。**整块搬而不是挑几个颜色**是有原因的:每套 61 个原始值是一整套两两校过对比度的关系,挑着抄必然出现"某个变量只在一个主题里定义了" —— 那一个主题下它渲染成透明,而且只有那一个主题。

**字体自托管**(`src/fonts/`,约 190KB):Inter(正文)、JetBrains Mono(代码)、Noto Serif SC(标题)。都是 OFL 1.1,可随软件分发,声明见 `src/fonts/NOTICE.md`。只放 latin 子集,中文回落系统字体(CJK 一套要 6MB,cc-haha 自己也是这么做的)。**不走 CDN** —— 这台服务常在内网/离线跑,外链的结果是字体直接不生效而不是慢一点。由 `/fonts/*.woff2` 路由提供,带一年不可变缓存。

#### 回合归服务端所有(断线保活)

这是最重要的一条改动。**以前**:生成的生命周期挂在那条 SSE 连接上,`res` 一 close 就 `abort()` 上游。于是手机锁屏、切后台、刷新页面、地铁里断一下网,正在生成的回答就没了,**已经烧掉的额度也白烧**。在移动端上这不是边缘情况,它就是常态。

**现在**(`src/chat_runs.js`):

- 回合由服务端持有,SSE 连接只是**订阅者**,来去自由;
- 最后一个订阅者离开后进入**宽限期**(`chatDisconnectGraceMs`,默认 5 分钟),期间生成继续;宽限期到了还没人回来才真的取消 —— **"没人在看"和"不要了"是两件事**;
- 只有显式点**「停止」**才等于"不要了",立刻取消上游;
- 每个事件带序号,重连时带 `from=<序号>` **续传**;序号太旧就先发一份快照(内容不缺,只是那一段没有逐字动画);
- 回合结束时的**落盘在 runner 的 finally 里做,与有没有订阅者无关** —— 这是"关掉页面再回来,回答还在"的唯一保证。

配 `chatDisconnectGraceMs: 0` 可退回旧语义(断开即取消)。进页面时如果这条会话还在生成,前端会自动接回去并提示一句;左栏列表里也会给正在生成的会话点一个脉冲小点 —— 断线保活之后,"另一个标签页里那条还在跑"是常态,列表是唯一能发现它的地方。

#### 其余能力

| 能力 | 说明 |
| --- | --- |
| 多会话 | 新建/切换/重命名/**置顶**/删除/**分叉**。**会话存服务端**(`<dataDir>/chats/<user>/`),换浏览器还在,清缓存不丢 |
| 会话列表 | **搜索**(⌘K)、按**时间分组**(置顶/今天/昨天/本周/本月/更早)、**摘要第二行**(标题常常是"帮我看个问题",光看标题分不出是哪个问题)。改名与置顶**不会**刷新排序时间 —— 否则改个标题就把三周前的对话顶到「今天」,而排序是列表唯一的导航方式 |
| AI 生成标题 | 第一轮说完后用最便宜的模型(haiku)生成一个 3~7 词的标题,**跟随你提问的语言**(用户用中文问、模型用英文答时,列表里出现英文标题是不对的)。手动改过标题的会话会上锁,AI 不再覆盖 |
| 用量条 | 侧栏底部常驻一行:**最紧的那一个额度**(有个人配额就是自己的,没有就是账户整体的 5 小时 / 7 天窗口),接近上限时变色,点开看完整明细。每轮结束后自动刷新 |
| 流式输出 | SSE 逐字出。生成中有一行**状态**:动词 + 已用时 + 已出 token("它还在动吗"必须一眼有答案,一个转圈图标回答不了) |
| 上游过载重试 | 429/529/5xx 时**退避重试**(默认 2 次,800ms → 1600ms),界面上给出第几次/状态码。**只在一个字都还没收到时重试** —— 已经吐了半段再重发会让用户看到两遍开头,那比直接报错更糟 |
| 消息操作 | 悬停出现:**复制** / **分叉**(从这条另起一个会话,原对话保留)/ **重新生成** / **改写提问**(编辑并重发,自动截断它之后的内容)。占位固定,出现时不推动版面;触屏上常驻 |
| 改写提问 | 替代的动作是"手动复制上一条 + 新开一段 + 把跑偏的回答删掉",三步变一步。空输入框按 **↑** 直接改写上一条 |
| 回合导航 | 左侧一条细标记条,每个提问一个标记,悬停看预览、点击跳过去;指针附近的标记加宽。长对话里"我刚才那个问题在哪"用滚动条是找不到的 |
| 上下文占用 | 顶栏一个小环 + 百分比,回答"我该不该开新对话了"。**分子取上游给过的真实 usage**(不是纯估算),不到 1% 时报 token 数而不是没信息量的「0.0%」 |
...
| 思考过程 | 折叠成**一行**,带尾巴预览:流式时跟着最新一句走(此刻在想什么),定稿后取第一行(它是这段推理的概括);第一行只是个冒号标题时取下一行。点开看全文,展开时自动跟到底 |
| 附件 | 图片 / **PDF** / **文本与代码**:点选、拖入、**直接粘贴**;按内容寻址存盘(同图去重),校验真身而不只看声明的 mime;点图**放大**看。见下方「附件」 |
| 联网 | **联网搜索** 与 **深度研究**(服务端工具,无需本地 agent 循环);查询词、来源、引用都摊在界面上并落盘。见下方「联网搜索与深度研究」 |
| 技能 | 多选**预设提示词**拼进 system(不是 Agent Skills,见下方说明) |
| Artifacts | HTML/SVG 在 **`<iframe sandbox>`** 里预览(给 `allow-scripts` 但**绝不给 `allow-same-origin`** —— 两者同时给等于没有沙箱);长代码块进侧栏带高亮;支持预览/源码切换、复制、下载 |
| 模型与深度 | 一个触发器 → 模型列表 → **浮在右侧的深度子菜单**(低/中/高·默认/很高/最大),父菜单留在原处,形状对齐 claude.ai;窄屏下子菜单盖在父菜单原位(390px 摆不下两列)。列表取自上游实际可用;**effort 只在支持的模型上出现**,thinking 选项按模型能力给(Fable 5 不接受 `disabled` 就摘掉该选项)。管理员给设备设了强制模型时,普通用户的选择器会锁定并说明,**管理员自己不受限** |
| 提示缓存 | 长对话自动打缓存断点,下一轮的前缀就能命中缓存。**短对话不打** —— 缓存写要 1.25 倍价,短对话打了是净亏(`chatPromptCache`) |
| 草稿 | 每个会话一份,切走再切回来还在 |
| 快捷键 | ⌘K 搜索会话、⌘⇧O 新对话、↑ 改写上一条、Esc 关覆盖层;发送键可在偏好里切「Enter」/「⌘Enter」 |
| 滚动 | **只有本来就贴着底才跟着滚**,往上翻看前文不会被一直拽回来;离底时出现「跳到最新」 |
| 并发保护 | 同一会话不并发:第二次发送返回 `409` 并带上可接回的回合信息,前端自动接过去而不是开第二条 |

#### 附件:图片 / PDF / 文本与代码

输入框的 **＋ → 添加文件或图片**,或直接拖入、粘贴。三类走三条完全不同的路:

| 类型 | 怎么进上下文 | 上限 |
| --- | --- | --- |
| 图片(png/jpeg/webp/gif) | `image` block(base64) | 5MB |
| **PDF** | `document` block(base64),**排在文字块前面**(API 要求的顺序) | 20MB |
| **文本与代码**(md/json/csv/js/ts/py/go/…) | 内联进消息正文的围栏块里,带文件名 | 1MB |

文本类不走 block 而是内联,有两个原因:模型对"带文件名的围栏块"理解最好,而且这样它能被**提示缓存**覆盖到(`document` block 不行)。

类型判定按**扩展名**而不是 mime —— 浏览器给 `.ts` / `.py` 的 mime 五花八门(`video/mp2t`、`text/x-python`、空串都见过),扩展名反而是稳定的那个。两类都验真身:PDF 要以 `%PDF-` 开头,文本要是合法 UTF-8 —— 二进制改个扩展名传上来,内联进正文就是一大片乱码,白烧一次额度才发现。附件与图片共用同一个内容寻址目录,也一起参与**引用清扫**(删了会话就不会永远留在盘上)。

#### 联网搜索与深度研究

靠 Anthropic 的**服务端工具**:工具跑在上游那边,所以这个转发代理**不需要任何本地 agent 循环**,也不需要工具执行环境。

| 模式 | 挂的工具 | 行为 |
| --- | --- | --- |
| **联网搜索** | `web_search`(≤5 轮) | 需要时自己去搜,回答带来源与引用 |
| **深度研究** | `web_search`(≤12 轮)+ `web_fetch`(抓原文,开引用) | 注入一段研究提示词(拆解问题 → 优先一手来源 → 摘要太薄就抓原文 → 交叉核对 → 说清哪些没查到),effort 默认拉到 `high` |

界面上会把过程摊开:生成时状态行显示「联网检索」,消息里一条折叠行列出**每一次的查询词**和命中的页面,正文末尾是编号的**来源清单**。"它到底搜了没有、搜的是什么"是联网回答里最该能核对的一件事,所以这些都跟消息一起落盘 —— 刷新页面还在。

两处实现上的坑,都是实测撞出来的:

- 用**基础版** `web_search_20250305`,不是新版 `web_search_20260209`。后者的动态过滤在底层走 code execution,这条订阅通路上实测直接 `too_many_requests`。
- **服务端工具失败不抛异常**:HTTP 仍是 200,只是结果块的 `content` 从**数组**变成一个错误**对象**。必须先分支再索引,否则失败会被当成"搜到了 0 条"静静吞掉。
- `pause_turn`:工具跑久了上游会先把这一段收尾。这不是结束 —— 要把已生成的内容回填成 assistant 轮再续一轮(最多 4 轮),不续的话联网回答经常只出半句话。

#### 「技能」= 预设提示词

**＋ → 技能** 可多选,选中的按顺序拼进那一轮的 `system`。两个来源:

| 来源 | 谁能建 | 谁看得到 | 存在哪 |
| --- | --- | --- | --- |
| **我的技能** | 每个用户自己(聊天页里「新建技能」) | **只有他自己** | `<dataDir>/chats/<user>/skills.json` |
| **团队技能** | 管理员(管理台「设置 → 技能库」) | 所有人 | `config.json` 的 `skills` |

**隔离是目录级的**:路径由 `safeSeg(user)` 推出,和会话、附件同一个目录、同一道关口 —— 读写都进不了别人的目录。用户名一律取服务端 session 推出来的,绝不采信请求里的任何用户标识。两个人可以用同一个 id 建各自的技能而互不影响;同 id 时**自己那份优先**(自己建的东西不该被全局配置悄悄盖掉)。每人上限 20 个。

选择器里两组分开列,自己的那些带「改」入口(可编辑/删除),团队的只读。`meta` 只下发 id / 名称 / 说明 / 来源,**提示词本体不出服务端**。

**这不是 Anthropic 的 Agent Skills**,界面上也这么写。真 Skills 要 `code_execution` 工具 + `container.skills` + `skills` scope,而**订阅 OAuth 凭证没有这个 scope** —— 实测 `GET /v1/skills` 明确回 `permission_error: OAuth token does not meet scope requirement`。所以这里的技能不会读你的文件、也不会跑脚本,就是一段提示词。用它做「代码审阅」「中文润色」「先问清需求再动手」这类固定人设是合适的。

#### 顺手修掉的一个真 bug:思考过程一直是空的

`thinking` 只传了 `{type:'adaptive'}`,没传 `display`。而 Opus 4.7 以后 `display` **默认是 `omitted`** —— 思考块照发,但文本是**空的**。界面上就是一个永远没内容的「思考过程」。现在显式要 `display:'summarized'`,实测思考文本正常返回。
...
| 配置项 | 默认 | 说明 |
| --- | --- | --- |
| `chatDisconnectGraceMs` | `300000` | 断线保活时长(毫秒)。`0` = 断开即取消(旧行为) |
| `chatMaxConcurrentTurns` | `3` | 同一用户同时进行的回合数上限 |
| `chatAutoTitle` | `true` | 第一轮结束后用便宜模型生成标题 |
| `chatPromptCache` | `true` | 长对话打提示缓存断点 |
| `chatMaxRetries` | `2` | 上游过载时的退避重试次数 |
| `chatMaxSessions` | `200` | 每用户会话数上限(磁盘保护),`0` = 不限 |
| `chatMaxMessages` | `500` | 每会话消息数上限(磁盘保护),`0` = 不限 |
...
"我还能不能再发一句"这个问题,过去只有管理员答得上来 —— 用户端只在**设了配额**时显示一条进度,没设配额就什么都不显示,而"没设配额"恰恰是最常见的情况。现在两种情况都有答案,给哪一层由**服务端**决定:

| 情况 | 看到的是 | 为什么 |
| --- | --- | --- |
| 管理员给这个用户**设了配额** | **他自己的额度**(已用 / 上限 / 窗口 / 重置时间) | 那是他实际会先撞上的那道墙 |
| **没设配额** | **账户整体额度**(订阅的 5 小时窗口、7 天窗口、按模型细分的窗口) | 没有个人上限时,他真正会撞上的是订阅账户自己的窗口 |

**"能看什么"落在服务端,不是前端 `hidden` 一下**:有个人配额时,`/u/api/quota` 的响应里**根本没有** `account` 字段 —— 前端藏起来的东西,一个 devtools 就翻出来了。

在哪看:

- **聊天页**侧栏底部常驻一行**用量条**(只报最紧的那一个数,接近上限时变色),点开是完整明细。每轮生成结束后自动刷一次 —— 刚烧掉的额度立刻反映出来;
- **`/u` →「我的设备」**的「用量合计」下面,同一份数据。

金额与百分比是**两件事**,分别管:

- 关掉「查看成本金额」权限(`perms.cost`)后,**金额**被抹成空,但**百分比照给** —— 比例不泄露金额,而"我还剩多少"必须仍然答得出来,否则这个人有一道限额却看不见它,撞上 429 时毫无预兆;
- 「**额外用量额度已用尽**」这条告警也**不跟着** `perms.cost` 走。它满了会让主窗口还有余量时的请求也吃 `529 Overloaded`,而官方那句报错只说 "Overloaded" —— 不点出来,没人查得到自己为什么失败。

> 「**没有兜底额度**」和「**兜底额度还剩很多**」是两种状态,不能画成同一根条。实测真实账号会回 `percent: 0` + `disabled_reason: out_of_credits` —— 照着 0% 画一根空条会被读成"还有余量",而事实是压根没有这份额度,主窗口用尽后只能等重置。界面上这两种分别说。

数据从哪来(`src/subscription_usage.js`,管理台的「订阅用量」面板同源、**共用同一份缓存**):

1. **首选**上游的订阅用量接口(与 Claude Code 的 `/usage` 同源),服务端缓存 10 分钟 —— 这个接口问得勤自己就会 429,所以**用户端传 `force=1` 也刷不动上游**,只有管理台的「刷新」按钮能穿透缓存;
2. 拿不到就**回落**到最近一次转发响应里的 `anthropic-ratelimit-unified-*` 头。注意那里是 **0~1 的比例**,而用量接口给的是 0~100 的百分比 —— 单位混了会把 48% 画成 0%;
3. 级联(`inherit`)模式下订阅额度属于上游那台 cc-trans,这台**看不到也不假装看得到**,直接说清楚。

> 限额头的采集在**每一条通往上游的路**上都要做。网页聊天走的是内部转发(不经过 `handleProxy`),漏掉那一处的症状是:只用网页聊天的用户永远看不到账户额度,而页面一点错都不报。
...
│   ├── hops.js            # 环路防护:跳数读写 / 官方上游剥头 / 超限判定
│   ├── upstream_auth.js   # 上游凭证三模式(oauth / apiKey / inherit)与热切换
│   ├── login_guard.js     # 登录失败节流:IP + 账号双维度、阶梯锁定、成功即清账
│   ├── users.js           # 普通用户数据层:scrypt 密码 / 令牌绑定 / 权限 / 配额 / 凭证版本
│   ├── user.js            # 用户端(/u)后端:独立 session、越权边界、公告、在线设备
│   ├── chat_runs.js       # 聊天回合的服务端所有权:订阅者、断线宽限期、按序号续传
│   ├── chat.js            # 聊天 HTTP 层:流式/续传/停止/分叉/改写/AI 标题/过载重试
│   ├── chat_store.js      # 会话与图片持久化(按用户分目录、内容寻址、引用清扫)
│   ├── md.js              # Markdown 渲染 + 代码高亮 + artifact 抽取(零依赖、可单测)
│   ├── storage.js         # 数据目录占用统计与清理
│   ├── admin.js           # Web 管理台后端:登录鉴权 + API + 托管页面
│   ├── admin-ui.html      # 管理台前端(单文件,原生 JS,零外部依赖)
│   ├── login-ui.html      # 统一登录页(/ 与 /login)
│   ├── user-ui.html       # 用户端前端
│   ├── chat-ui.html       # 聊天前端(/u/chat 与 /admin/chat 共用同一份)
│   ├── ui-tokens.css      # /admin、/u、登录页共享的设计令牌
│   ├── haha-tokens.css    # 聊天页的设计令牌:cc-haha「纸 · 墨 · 印」六套色板 + 语义层
│   └── fonts/             # 自托管字体(Inter / JetBrains Mono / Noto Serif SC,OFL)
├── deploy/
...
│   ├── login.mjs          # 统一登录入口 / 走错门指路 / 登录节流 / 公告 / 会话作废
│   ├── chat2.mjs          # 断线保活与续传 / 停止 / 分叉 / 改写 / AI 标题 / 过载重试
│   ├── ui.mjs             # 真浏览器(Chrome CDP)跑三个页面(npm run test:ui)
...
这个项目的一部分设计是从别的开源项目里学来的。列在这里,一是致谢,二是让"为什么这么做"有个出处可查。

### [cc-haha](https://github.com/NanmiCoder/cc-haha)(桌面端 Claude Code 工作台)

它是 Electron + React 的桌面应用,栈和这里完全不同(本项目是零构建的单文件 HTML),所以照搬的是**机制与交互决策**,不是代码:

| 借来的 | 它解决的问题 | 这里的落点 |
| --- | --- | --- |
| **会话由服务端持有,客户端只是订阅者**(它的 `ws/handler.ts` 里那套 disconnect grace + active-work watcher) | 手机锁屏、切后台、断网,不该打断正在跑的任务 | `src/chat_runs.js`:宽限期 + 按序号续传 + 落盘与订阅者无关 |
| **"没人在看" ≠ "不要了"** | 关掉连接就 abort 上游,等于把已花的额度扔掉 | 只有显式 `/stop` 才取消;断开只是取消订阅 |
| `ThinkingBlock` 的**一行折叠 + 尾巴预览** | "思考过程 ▸" 这种空标题什么也没告诉你 | 流式时跟最新一句、定稿后取第一行(第一行是冒号标题时取下一行) |
| `StreamingIndicator`:**动词 + 已用时 + 已出 token** | 一个转圈图标回答不了"它还在动吗" | 生成中的状态行;上游过载重试时换成重试横幅 |
| `MessageActionBar` / `GitFork` | 复制、分叉、重新生成、改写要在**每条消息**上就近可达 | 悬停出现的操作条(占位固定,触屏常驻) |
| `ConversationNavigator` | 长对话里"我刚才那个问题在哪"用滚动条找不到 | 左侧标记条,悬停预览、点击跳转、指针附近加宽 |
| `ContextUsageIndicator` | "我该不该开新对话了"此前完全没有信号 | 顶栏小环;分子用上游给过的真实 usage 而非纯估算 |
| `titleService` 的**两段式标题** | 截断第一句当标题,列表里全是"帮我看个问题" | 先截断顶上,第一轮说完后用 haiku 换成 3~7 词标题,**跟随提问的语言** |
| `sendShortcut` | Enter 发送 vs ⌘Enter 发送是个人偏好 | 偏好设置里可切 |
| `TurnCompletionStamp` | 最后一条回复的时间与耗时最常被看 | 时间戳常驻在最后一条上 |
| **整套视觉体系**(`theme/globals.css` 的六套色板 + 语义层、字体角色、圆角刻度) | 观感要一致,就不能只搬机制 | `src/haha-tokens.css`;聊天页的每一处颜色都走 `--color-*`,没有写死色值 |

**没有照搬的**:它的 Diff 审阅、内置浏览器、权限审批、MCP/SubAgent 管理、Agent Teams、Workflow 编排、Computer Use、技能市场 —— 那些是"给 Claude Code 做工作台"的能力,而这里是一个转发代理附带的聊天界面,搬进来只会让两件事都做不好。

### [sub2api](https://github.com/Wei-Shaw/sub2api)(Go + Vue 的 AI API 网关)

它是面向运营的重型平台(Postgres + Redis + 支付 + 多渠道监控),规模和定位都和这个自用工具不同。借来的是它在**账号安全**上的几个做法:

| 借来的 | 它解决的问题 | 这里的落点 |
| --- | --- | --- |
| **登录接口的失败节流**(它用 Redis + fail-close) | 三个登录接口此前都能被无限次高速试密码。scrypt 让单次变慢 60ms,但并发 50 条连接仍是每秒近千次 | `src/login_guard.js`:纯内存实现,IP + 账号双维度、阶梯锁定、成功即清账 |
| **`TokenVersion`:改密码即作废所有会话** | "改完密码,另一台电脑上还登着"是个很容易漏的老问题 | `users.js` 的 `passVersion`;会话记住签发版本,对不上即失效。管理台凭证同样有一份 |
| **认证事件入审计** | 登录成功/失败/锁定要能事后查 | 统一走 `[audit]` 前缀,带账号、IP、UA |
| **公告** | 自建服务里"今晚重启"只能靠口头通知 | 管理台设置页写一句,用户端与聊天页顶部显示,可关、改了重现 |

**没有照搬的**:注册流程、邮箱验证、OAuth 第三方登录(GitHub/Google/微信/钉钉/LinuxDo)、Passkey、TOTP 二步验证、支付与兑换码、渠道监控与故障转移。这是个**账号由管理员手工开通的自用工具**,一共可能就三五个账号 —— 把注册和支付搬进来是给自己找活干。TOTP 是唯一有点想要的,但它需要一整套备份码/恢复流程才不至于把人锁在门外,不值得为三五个账号做。

### claude-relay-service

参数下发、身份伪装、限流与白名单那一组能力(见上文「进阶能力」)。
